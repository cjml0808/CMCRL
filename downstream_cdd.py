...
from citrus_data_7 import CitrusDisease7, CitrusDisease4, CDD
...
def eval(mode='full_finetune'):
    ...
    num_class = 7
    model = models.create('resnet_ibn50a_ori', num_features=0, norm=True, dropout=0,
    ...
    if mode == 'final_finetune':
        logging.basicConfig(filename=os.path.join(PATH, f"2048_final_testing_{checkpoint['epoch']}_normalize___.log"),
                            level=logging.DEBUG)
        ...
        print(f"Best_top5_accuracy: {top5_accuracy}.")
        print(f"Best_f1_score: {f1_test}.")
        print(f"Best_recall_score: {r_test}.")
        print(f"Best_precision_score: {p_test}.")
        print(f"Total time: {all_time}.")
        logging.info(f"Best_top1_accuracy: {top1_accuracy}.")
        logging.info(f"Best_top5_accuracy: {top5_accuracy}.")
        logging.info(f"Best_f1_score: {f1_test}.")
        logging.info(f"Best_recall_score: {r_test}.")
        logging.info(f"Best_precision_score: {p_test}.")
